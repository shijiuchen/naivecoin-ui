<template>
  <div>
    ...
    <div style="background: whitesmoke;font-size: 15px">
      <div v-if="taskName.length !== 0 "><div v-if="this.taskName==='asylo'">开始执行任务<p class="text" style="font-size: 15px; color: #ad1b22">computeMatrix</p></div><div v-else>开始执行任务<p class="text" style="font-size: 15px; color: #ad1b22" v-html="taskName"></p></div></div>
      <br>
      <div v-if="taskName === 'hadoop' ">您被选为<p class="text" style="font-size: 15px; color: #8e44ad">master节点</p></div>

      <div v-if="taskName.length === 0 "><span>暂无任务执行......</span></div>

      <br>
      <div v-if="taskTime1.length !== 0 && taskTime2.length !== 0 && this.progress===100"><div v-if="this.taskName==='asylo'">执行任务computeMatrix的时间为：</div><div v-else>执行任务{{taskName}}的时间为：</div><p class="text" style="font-size: 15px; color: #1c7430" v-html="parseInt(taskTime2)-parseInt(taskTime1)">ms</p></div>
      <div v-if="taskTime1.length === 0 || taskTime2.length === 0 || this.progress!==100"><span>暂无执行时间返回......</span></div>
      <br>
      <div class="divcss5-a" v-if="taskResult.length !== 0 && this.progress===100"><div v-if="this.taskName==='asylo'">执行任务computeMatrix的结果为：</div><div v-else>执行任务{{taskName}}的结果为：</div><p class="text" style="font-size: 15px; color: deepskyblue" v-html="taskResult"></p></div>
      ...
      <div v-if="taskExenMaster.length !== 0 && taskExenWorker1.length !== 0 && taskExenWorker2.length !== 0">该任务执行工作量为:<br>master:{{taskExenMaster}}(cores+MB)*s;<br>worker1:{{taskExenWorker1}}(cores+MB)*s;<br>worker2:{{taskExenWorker2}}(cores+MB)*s</div>

      <div v-if="taskExenWorker.length !== 0 ">执行任务hadoop<br>您是<p class="text" style="font-size: 15px; color: #8e44ad">worker节点</p><br>该任务执行工作量为:{{taskExenWorker}}(cores+MB)*s</div>
...
//            this.init();
      ...
      timeTogetResults(){
        ...
        this.interval3=setInterval(this.gettaskResult,15000);
        this.interval4=setInterval(this.getBegintaskTime,15000);
        this.interval5=setInterval(this.getEndtaskTime,15000);
        ...
        this.interval10=setInterval(this.getTransactionPool,2000);
      },
